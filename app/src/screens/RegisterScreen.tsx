...
export function RegisterScreen() {
  ...
  const sendCode = async () => {
    try {
      setBusy(true);
      const { data } = await api.post('/auth/send-otp', { phone: phone.trim(), purpose: 'REGISTER' });
      setStep('otp');
  ...
  const register = async () => {
    try {
      setBusy(true);
      await api.post('/auth/verify-otp', { phone: phone.trim(), purpose: 'REGISTER', code });
      const { data } = await api.post('/auth/register', { phone: phone.trim(), password, username: username.trim(), displayName: displayName.trim(), dateOfBirth, termsAccepted });
  ...
  return <Screen><ScrollView keyboardShouldPersistTaps="handled">
    ...
    <Text style={{color:colors.muted,marginBottom:16}}>You must be 18 or older. Use a Bangladesh mobile number.</Text>
    <Field placeholder="Phone (01XXXXXXXXX)" keyboardType="phone-pad" editable={step==='details'} value={phone} onChangeText={setPhone}/>
    {step === 'details' ? <>
      ...
      <Button title={busy?'Sending…':'Send verification code'} disabled={busy || !/^01\d{9}$/.test(phone) || username.length<3 || !displayName || password.length<8 || !dateOfBirth || !termsAccepted} onPress={sendCode}/>
    </> : <>
