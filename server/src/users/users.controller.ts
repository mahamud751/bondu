...
export class UsersController {
  ...
  @Get('me') me(@CurrentUser() user: { sub: string }) { return this.db.user.findUnique({ where: { id: user.sub }, select: { id: true, phone: true,email:true, role: true, status: true, dateOfBirth: true, profile: true, vendor:{select:{id:true,status:true,legalName:true,address:true,contactEmail:true,profileDescription:true,supportedLanguages:true,preferredWorkingHours:true,payoutMethod:true,payoutAccountLast4:true,identityProvider:true,identityStatus:true,commissionPercent:true,voiceRatePerMinute:true,videoRatePerMinute:true,voiceCallEnabled:true,videoCallEnabled:true,paidChatRate:true,availableForCall:true,breakActive:true,autoAcceptCalls:true,maximumDailyCalls:true,minimumCallerBalance:true,averageRating:true,approvedAt:true}}, createdAt: true } }); }
